...
const StrategyExecution = ({ title, steps }) => {
    const displayTitle = title || "Where Strategy Meets Execution That <br /> <span>Turning Vision Into Action</span>";
    const displaySteps = steps || processSteps;

    return (
        <section className={styles.wrapper}>
            <div className={styles.container}>
                <h2 className={styles.mainTitle} dangerouslySetInnerHTML={{ __html: displayTitle }}></h2>
                    ...
                    {displaySteps.map((step, index) => (
                        <div key={step.id} className={styles.timelineItem}>
                            ...
                            <div className={styles.indicatorCol}>
                                ...
                                {index !== displaySteps.length - 1 && <div className={styles.verticalLine}></div>}
                            </div>

...
const UniversalGridEditor = () => {
    ...
    const arrayKey = content.challenges ? 'challenges' : content.features ? 'features' : 'items';
    const currentItems = content[arrayKey] || [];

    const updateItemField = (index, field, value) => {
        const newItems = [...currentItems];
        newItems[index] = { ...newItems[index], [field]: value };
        setContent(prev => ({ ...prev, [arrayKey]: newItems }));
    };

    const addItem = () => {
        setContent(prev => ({
            ...prev,
            [arrayKey]: [...currentItems, { icon: 'BsGear', title: 'New Item', desc: '' }]
        }));
    };
    ...
    const hasSubheading = 'subheading' in content || arrayKey === 'features';
    const hasMainTitle = 'mainTitle' in content;
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 className="text-white fw-bold mb-1">Grid List Editor</h2>
            ...
            <div style={sectionStyle}>
                <Form.Group className="mb-4">
                    <Form.Label style={labelStyle}>{hasMainTitle ? "Main Title" : "Section Heading"}</Form.Label>
                    <Form.Control
                        type="text"
                        name={hasMainTitle ? "mainTitle" : "heading"}
                        value={hasMainTitle ? (content.mainTitle || "") : (content.heading || "")}
                ...
                {hasSubheading && (
                    <Form.Group className="mb-5">
                        <Form.Label style={labelStyle}>Subheading</Form.Label>
                        <Form.Control
                            type="text"
                            name="subheading"
                ...
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <h5 className="mb-0 fw-bold text-white"><ListUl className="me-2" /> Grid Items ({currentItems.length})</h5>
                    <Button
                        ...
                        <Col md={6} key={idx}>
                            <div style={itemCardStyle}>
                                ...
                                <Row className="g-3">
                                    ...
                                    {arrayKey === 'features' && (
                                        <Col md={12}>
                                            <Form.Label style={labelStyle}>Link (Optional)</Form.Label>
                                            <Form.Control
                                                style={inputStyle}
                                                value={item.link || ""}
...
export default UniversalGridEditor;
